...
| ملفات اختبار في `test/` | **32+** (طبقة الوكيل + الملفات + الخصوصية + المالية) |
| ملفات بيانات طبية (JSON) | 5 — معلنة الآن في `pubspec.yaml` تحت `lib/data/medical/` |
| ملفات ترجمة (ARB) | 2 (عربي/إنجليزي) |
| مجلدات الميزات المستقلة | 23 |
| شاشات المستخدم الكاملة | 11 (افتتاحية، إعداد أولي، رئيسية، ملف صحي، إعدادات، محفظة، مركز AI، وكيل ذكي، مساعد بصري، خصوصية، عائلة) |
...
| 9 | شاشة خصوصية مخصصة | `privacy_settings_screen.dart` | ✅ مربوطة بـ `ProfilePrivacyManager` |
| 10 | شاشة إدارة العائلة | `family_management_screen.dart` | ✅ مربوطة بـ `ActiveProfileController.addFamilyMember` |
...
- شاشة خصوصية مخصصة (TODO #9) — **تم التنفيذ**: `privacy_settings_screen.dart` مربوطة بـ `ProfilePrivacyRegistry`/`ProfilePrivacyManager`
- شاشة إدارة العائلة الكاملة (TODO #10) — **تم التنفيذ**: `family_management_screen.dart` مربوطة بـ `ActiveProfileController.addFamilyMember`
...
`AiAgentScreen` (`lib/screens/ai_agent_screen.dart`) — وضعا "محادثة" (رد مباشر عبر `AiServiceRouter`) و"وكيل" (تخطيط متعدد الخطوات عبر `CoordinatorAgent.handleUserRequest`)، مع شريط تقدم ✓/●/○، زر إيقاف، وعرض مستوى الثقة + التنويه الإلزامي. **نقطة الدخول مربوطة**: زر «الوكيل الذكي» في `home_screen.dart` وبطاقة في `ai_hub_screen.dart`، بنفس نمط `MaterialPageRoute` المستخدم لبقية الشاشات.
